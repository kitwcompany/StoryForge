...
pub fn list_stories(
    pool: State<'_, DbPool>,
) -> Result<Vec<crate::db::Story>, AppError> {
...
pub fn create_story(
    ...
    let story = StoryRepository::new(pool.inner().clone())
        .create(CreateStoryRequest { title, description, genre, style_dna_id: None })
        .map_err(AppError::from)?;
...
pub fn update_story(
    ...
    StoryRepository::new(pool.inner().clone()).update(&id, &req).map_err(AppError::from)?;
...
pub fn delete_story(
    ...
    StoryRepository::new(pool.inner().clone()).delete(&id).map_err(AppError::from)?;
