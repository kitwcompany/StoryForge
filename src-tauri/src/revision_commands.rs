...
use crate::scene_commands::create_version_snapshot;
...
pub async fn track_change(
    ...
    let story_id: Result<String, rusqlite::Error> = if let Some(ref sid) = scene_id {
        conn.query_row("SELECT story_id FROM scenes WHERE id = ?1", [sid], |row| row.get(0))
    } else if let Some(ref cid) = chapter_id {
        conn.query_row("SELECT story_id FROM chapters WHERE id = ?1", [cid], |row| row.get(0))
    } else {
        Err(rusqlite::Error::InvalidQuery)
    };
    if let Ok(story_id) = story_id {
        let _ = crate::state_sync::StateSync::emit_data_refresh(&app_handle, Some(&story_id), "changeTracks");
    }
...
pub async fn create_comment_thread(
    ...
    let result = repo.create_thread(&thread)
        .map_err(|e| {
    ...
    if let Ok(story_id) = story_id {
        let _ = crate::state_sync::StateSync::emit_data_refresh(&app_handle, Some(&story_id), "commentThreads");
    }
...
pub async fn add_comment_message(
    ...
    let repo = CommentThreadRepository::new(pool.inner().clone());
    let result = repo.add_message(&message)
        .map_err(|e| {
...
pub async fn resolve_comment_thread(
    ...
    let repo = CommentThreadRepository::new(pool.inner().clone());
    let result = repo.resolve_thread(&thread_id)
        .map_err(AppError::from)?;
...
pub async fn reopen_comment_thread(
    ...
    let repo = CommentThreadRepository::new(pool.inner().clone());
    let result = repo.reopen_thread(&thread_id)
        .map_err(AppError::from)?;
...
pub async fn delete_comment_thread(
    ...
    let repo = CommentThreadRepository::new(pool.inner().clone());
    let result = repo.delete_thread(&thread_id)
        .map_err(AppError::from)?;
